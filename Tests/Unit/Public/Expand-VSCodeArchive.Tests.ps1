...
Describe "Expand-VSCodeArchive" -Tag 'Unit', 'Public' {

    Context "Parameter Validation" {
        BeforeEach {
            # Create a dummy archive file
            $archivePath = Join-Path $TestDrive "VSCode-win32-x64-1.85.0.zip"
            "dummy archive" | Out-File -LiteralPath $archivePath
        }

        It "Should accept valid DownloadDirectory parameter" {
            { Expand-VSCodeArchive -DownloadDirectory $TestDrive -WhatIf } | Should -Not -Throw
        }
        ...
        It "Should support -Confirm parameter" {
            { Expand-VSCodeArchive -DownloadDirectory $TestDrive -Confirm:$false -WhatIf } | Should -Not -Throw
        }

        It "Should accept -Force parameter" {
            { Expand-VSCodeArchive -DownloadDirectory $TestDrive -Force -WhatIf } | Should -Not -Throw
        }

        It "Should accept -AddPath parameter" {
            { Expand-VSCodeArchive -DownloadDirectory $TestDrive -AddPath -WhatIf } | Should -Not -Throw
        }
    }
    ...
    Context "AddPath Integration" {
        ...
        It "Should not call Add-Path when using -WhatIf" {
            Expand-VSCodeArchive -DownloadDirectory $TestDrive -AddPath -WhatIf
    ...
    Context "Integration with Class" {
        ...
        It "Should use DownloadDirectory when specified" {
            ...
            $archivePath = Join-Path $customDir "VSCode-win32-x64-1.85.0.zip"
            "dummy archive" | Out-File -LiteralPath $archivePath

            { Expand-VSCodeArchive -DownloadDirectory $customDir -WhatIf } | Should -Not -Throw
        }
    ...
    Context "Return Value" {
        ...
        It "Should return destination path with -WhatIf" {
            $result = Expand-VSCodeArchive -DownloadDirectory $TestDrive -WhatIf
